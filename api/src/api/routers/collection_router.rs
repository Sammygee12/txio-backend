...
pub fn router(service: CollectionService) -> Router {
    Router::new()
        ...
        .route(
            "/{id}/requests",
            get(collection_handler::get_collection_requests),
        )
        .route(
            "/{id}/requests/{req_id}",
            put(collection_handler::update_request),
        )
        .route(
            "/{id}/requests/{req_id}",
            delete(collection_handler::delete_request),
        )
        .route(
            "/{id}/requests/{req_id}/execute",
            post(collection_handler::execute_request),
        )
        .with_state(service)
}
